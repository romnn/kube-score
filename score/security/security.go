...
func Register(allChecks *checks.Checks, options Options) {
	allChecks.RegisterPodCheck("Container Security Context User Group ID", `Makes sure that all pods have a security context with valid UID and GID set `, containerSecurityContextUserGroupID(options))
	allChecks.RegisterPodCheck("Container Security Context Privileged", "Makes sure that all pods have a unprivileged security context set", containerSecurityContextPrivileged(options))
	...
	allChecks.RegisterOptionalPodCheck("Container Seccomp Profile", `Makes sure that all pods have at a seccomp policy configured.`, podSeccompProfile(options))
}

// containerSecurityContextReadOnlyRootFilesystem checks for pods using writeable root filesystems
func containerSecurityContextReadOnlyRootFilesystem(options Options) func(ps ks.PodSpecer) (score scorecard.TestScore, err error) {
...
func containerSecurityContextPrivileged(options Options) func(ps ks.PodSpecer) (score scorecard.TestScore, err error) {
	return func(ps ks.PodSpecer) (score scorecard.TestScore, err error) {
		var allContainers []corev1.Container
		if !options.SkipInitContainers {
			allContainers = append(allContainers, ps.GetPodTemplateSpec().Spec.InitContainers...)
...
func containerSecurityContextUserGroupID(options Options) func(ps ks.PodSpecer) (score scorecard.TestScore, err error) {
	return func(ps ks.PodSpecer) (score scorecard.TestScore, err error) {
		var allContainers []corev1.Container
		if !options.SkipInitContainers {
			allContainers = append(allContainers, ps.GetPodTemplateSpec().Spec.InitContainers...)
...
func podSeccompProfile(options Options) func(ps ks.PodSpecer) (score scorecard.TestScore, err error) {
	return func(ps ks.PodSpecer) (score scorecard.TestScore, err error) {
