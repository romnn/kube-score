...
func TestStableVersionIngress(t *testing.T) {
	...
	scoreNew, _ := newKubernetes(
		ks.BothMeta{
			TypeMeta: v1.TypeMeta{Kind: "Ingress", APIVersion: "extensions/v1beta1"},
		},
	)
	assert.Equal(t, scorecard.GradeWarning, scoreNew.Grade)
	assert.Equal(
		...
		[]scorecard.TestScoreComment{
			{
				Path:             "",
				Summary:          "The apiVersion and kind extensions/v1beta1/Ingress is deprecated",
				Description:      "It's recommended to use networking.k8s.io/v1 instead which has been available since Kubernetes v1.19",
				DocumentationURL: "",
			},
		},
		scoreNew.Comments,
...
func TestStableVersionPodDisruptionBudget(t *testing.T) {
	...
	assert.Equal(
		...
		[]scorecard.TestScoreComment{
			{
				Path:             "",
				Summary:          "The apiVersion and kind policy/v1beta1/PodDisruptionBudget is deprecated",
				Description:      "It's recommended to use policy/v1 instead which has been available since Kubernetes v1.21",
				DocumentationURL: "",
			},
		},
		scoreNew.Comments,
...
func TestStableNetworkingIngress(t *testing.T) {
	...
	assert.Equal(
		...
		[]scorecard.TestScoreComment{
			{
				Path:             "",
				Summary:          "The apiVersion and kind networking.k8s.io/v1beta1/Ingress is deprecated",
				Description:      "It's recommended to use networking.k8s.io/v1 instead which has been available since Kubernetes v1.19",
				DocumentationURL: "",
			},
		},
