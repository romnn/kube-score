	...
	for _, spread := range spreads {
		if spread.LabelSelector == nil {
			...
			score.AddComment(
				"",
				"Pod Topology Spread Constraints",
				"No labelSelector detected. A label selector is needed determine the number of pods in a topology domain",
			)
			return
		}
		...
		if spread.MaxSkew == 0 {
			...
			score.AddComment(
				"",
				"Pod Topology Spread Constraints",
				"MaxSkew is set to zero. This is not allowed.",
			)
			return
		}
		...
		if spread.MinDomains != nil && *spread.MinDomains == 0 {
			...
			score.AddComment(
				"",
				"Pod Topology Spread Constraints",
				"MaxDomain set to zero. This is not allowed. Constraint behaves if minDomains is set to 1 if nil",
			)
			return
		}
		...
		if spread.TopologyKey == "" {
			...
			score.AddComment(
				"",
				"Pod Topology Spread Constraints",
				"TopologyKey is not set. This is the key of node labels used to bucket nodes into a domain",
			)
			return
		}
		...
		if spread.WhenUnsatisfiable != "DoNotSchedule" &&
			...
			score.AddComment(
				"",
				"Pod Topology Spread Constraints",
				"Invalid WhenUnsatisfiable setting detected",
			)
			return
		}
	}
